...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnFirstTwoHiredEmployees()
    {
        ...
        var firstTwo = emps.OrderByDescending(emp => emp.HireDate).Take(2).ToList(); 
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(emp => new {EName = emp.EName, Total = emp.Sal + (emp.Comm != null ? emp.Comm : 0) }); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            .SelectMany(x => grades
                    .Where(grade => x.emp.Sal >= grade.Losal && x.emp.Sal <= grade.Hisal),
                (x, grade) => new
                {
                    EName = x.emp.EName,
                    DName = x.dept.DName,
