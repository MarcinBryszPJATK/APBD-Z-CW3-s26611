...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(emp => depts.Any(dept => dept.Loc == "CICAGO" && emp.DeptNo == dept.DeptNo)).ToList(); 
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo, dept => dept.DeptNo, (emp, dept) => new {EName = emp.EName, DName = dept.DName}); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.SelectMany(emp =>emp.Comm != null ? new[] { (EName: emp.EName, Comm: emp.Comm) } : Enumerable.Empty<(string EName, decimal? Comm)>() );
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(emp => grades.Where(grade => emp.Sal >= grade.Losal && emp.Sal <= grade.Hisal), (emp, grade)=> new {EName = emp.EName, Grade = grade.Grade});
